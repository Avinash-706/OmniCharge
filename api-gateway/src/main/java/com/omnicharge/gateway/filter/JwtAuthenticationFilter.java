    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            if (Boolean.FALSE.equals(isProfileComplete) && !isProfileIncompleteAllowedPath(path)) {
                ...
                exchange.getResponse().getHeaders().add("X-Error-Reason", "INCOMPLETE_PROFILE");
                return exchange.getResponse().setComplete();
            }
            ...
            return reactiveRedisTemplate.hasKey(blacklistKey)
                    .flatMap(isBlacklisted -> {
                        ...
                        String deactivationKey = "user:deactivated:" + userId;
                        return reactiveRedisTemplate.hasKey(deactivationKey)
                                .flatMap(isDeactivated -> {
                                    if (Boolean.TRUE.equals(isDeactivated)) {
                                        // User has been deactivated by admin
                                        logAuthenticationFailure(path, "USER_DEACTIVATED", 
                                            "User account has been deactivated by administrator", 
                                            sanitizeTokenForLogging(token, userId, email));
                                        exchange.getResponse().getHeaders().add("X-Error-Reason", "USER_DEACTIVATED");
                                        exchange.getResponse().setStatusCode(HttpStatus.FORBIDDEN);
                                        return exchange.getResponse().setComplete();
                                    }

                                    // Token is valid and user is active, add user info to headers for downstream services
                                ...
                                .onErrorResume(e -> {
                                    // Redis error during deactivation check - allow request to proceed (fail open)
                                    log.warn("Redis error during deactivation check, allowing request: {}", e.getMessage());
                                    ServerHttpRequest modifiedRequest = request.mutate()
                                            .header("X-User-Id", claims.get(JWT_CLAIM_USER_ID, String.class))
                                            .header("X-User-Role", claims.get(JWT_CLAIM_ROLE, String.class))
                                            .header("X-User-Email", claims.get(JWT_CLAIM_EMAIL, String.class))
