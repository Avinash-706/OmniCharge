...
class InternalRechargeControllerTest {
    ...
    @Test
    void getExpiringRecharges_CustomDaysLeft() throws Exception {
        ExpiringRechargeResponse response = ExpiringRechargeResponse.builder()
                .rechargeId("OMNI-EXPIRING2")
                .userId(2L)
        ...
        when(rechargeService.getExpiringRecharges(3)).thenReturn(Collections.singletonList(response));

        mockMvc.perform(get("/api/internal/recharges/expiring?daysLeft=3"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data[0].operatorName").value("Jio"));
    }

    @Test
    void getExpiringRecharges_DefaultDaysLeft() throws Exception {
        when(rechargeService.getExpiringRecharges(5)).thenReturn(Collections.emptyList());

        mockMvc.perform(get("/api/internal/recharges/expiring"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    @Test
    void getExpiredToday_EmptyList() throws Exception {
        when(rechargeService.getExpiredToday()).thenReturn(Collections.emptyList());
