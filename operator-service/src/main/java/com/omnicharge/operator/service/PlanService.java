...
import com.omnicharge.operator.dto.PlanStatsResponse;
...
public class PlanService implements IPlanService {
    ...
    public PlanResponse createPlan(Long operatorId, PlanRequest request) {
        ...
        // Log business operation with resilient error handling
        try {
            ...
            context.put("validityDays", plan.getValidityDays());
            publishBusinessLog("PLAN_CREATED",
                "Plan created: planId=" + plan.getId() + ", name=" + plan.getPlanName() + ", operator=" + operator.getName(),
                context);
        } catch (Exception e) {
            log.warn("[LOGGING FALLBACK] Failed to publish plan creation log: {}", e.getMessage());
            log.info("[FALLBACK LOG] eventType=PLAN_CREATED, planId={}, planName={}, operatorName={}", 
                     plan.getId(), plan.getPlanName(), operator.getName());
        }

        // CRITICAL: Evict cache immediately
        systemCacheService.evictPlanCache(operatorId);
        systemCacheService.evictPlanDetailCache(plan.getId());
    ...
    public PlanResponse updatePlan(Long planId, PlanRequest request) {
        ...
        // CRITICAL: Evict cache immediately
        systemCacheService.evictPlanCache(plan.getOperator().getId());
        systemCacheService.evictPlanDetailCache(planId);
    ...
    public PlanResponse activatePlan(Long planId) {
        ...
        if (!plan.getOperator().getIsActive()) {
            throw new IllegalStateException(
                "Cannot activate plan: operator '" + plan.getOperator().getName() + 
                "' is inactive. Please activate the operator first."
            );
        }
    ...
    public PlanStatsResponse getPlanStats() {
        ...
        long inactivePlans = planRepository.countByIsActive(false);
        
        // Get plans by category (only active plans)
        List<Object[]> categoryData = planRepository.countActivePlansByCategory();
        Map<String, Long> plansByCategory = categoryData.stream()
                .collect(Collectors.toMap(
                        row -> ((PlanCategory) row[0]).name(),
                        row -> (Long) row[1]
                ));
        
        log.info("Plan stats generated: totalPlans={}, activePlans={}, inactivePlans={}", 
