...
public class SystemCacheService {
    ...
    /**
     * Evict plan cache for a specific operator.
     * Called when plans are activated/deactivated or operator status changes.
     */
    public void evictPlanCache(Long operatorId) {
        try {
            String cacheKey = "plans:operator:" + operatorId;
            redisTemplate.delete(cacheKey);
            log.info("Evicted plan cache for operator: {}", operatorId);
        } catch (Exception e) {
            log.warn("Failed to evict plan cache for operator {}: {}", operatorId, e.getMessage());
        }
    }

    /**
     * Evict specific plan detail cache.
     * Called when a plan is updated or deleted.
     */
    public void evictPlanDetailCache(Long planId) {
        try {
            String detailKey = "plan:detail:" + planId;
            redisTemplate.delete(detailKey);
            log.info("Evicted plan detail cache for plan: {}", planId);
        } catch (Exception e) {
            log.warn("Failed to evict plan detail cache for plan {}: {}", planId, e.getMessage());
        }
    }
