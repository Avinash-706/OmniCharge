...
class UserServiceBusinessOperationPropertyTest {
    ...
    void property_loginAttempt_shouldLogOutcomeAndReason() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            when(jwtUtil.generateAccessToken(anyLong(), anyString(), anyString(), anyBoolean(), anyBoolean(), anyString())).thenReturn("access-token");
    ...
    void property_passwordResetRequest_shouldLogWithUserId() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            user.setEmail(request.getEmail());
            user.setAuthProvider(AuthProvider.LOCAL);
            user.setMobileNumber("9876543210"); // Set mobile number
            user.setIsMobileVerified(true); // Set mobile as verified (required for password reset)
