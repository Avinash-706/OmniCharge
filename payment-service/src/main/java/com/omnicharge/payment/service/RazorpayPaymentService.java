...
public class RazorpayPaymentService implements IRazorpayPaymentService {
    ...
    public RazorpayPaymentService(RazorpayClient razorpayClient, LogEventPublisher logEventPublisher) {
    ...
    @Override
    public String fetchPaymentIdForOrder(String orderId) throws RazorpayException {
        java.util.List<Payment> payments = razorpayClient.orders.fetchPayments(orderId);
        if (payments != null) {
            for (Payment p : payments) {
                String status = p.get("status");
                if ("captured".equalsIgnoreCase(status) || "authorized".equalsIgnoreCase(status) || "paid".equalsIgnoreCase(status)) {
                    return p.get("id");
                }
            }
        }
