...
class NotificationControllerTest {
    ...
    void getUserNotifications_EmptyPage() throws Exception {
        Page<NotificationResponse> emptyPage = new PageImpl<>(Collections.emptyList());
        when(notificationService.getUserNotifications(eq(10L), any(Pageable.class))).thenReturn(emptyPage);
    ...
    void getUserNotifications_WithPagination() throws Exception {
        NotificationResponse resp = NotificationResponse.builder()
                .id(1L).userId(10L).type(NotificationType.SMS).category(NotificationCategory.PAYMENT_SUCCESS)
                .subject("Recharge").message("Done").status(NotificationStatus.SENT)
                .referenceId("REC-1").isRead(true).createdDate(LocalDateTime.now()).build();
    ...
    @Test
    void markAsRead_WithDifferentNotificationId() throws Exception {
        doNothing().when(notificationService).markAsRead(99L, 10L);
        ...
        verify(notificationService).markAsRead(99L, 10L);
    }
