...
class AdminNotificationControllerTest {
    ...
    @Test
    void getAllNotifications_EmptyPage() throws Exception {
        Page<NotificationResponse> emptyPage = new PageImpl<>(Collections.emptyList());
        when(notificationService.getAllNotifications(any(Pageable.class))).thenReturn(emptyPage);
    ...
    @Test
    void getAllNotifications_WithPagination() throws Exception {
        NotificationResponse resp = NotificationResponse.builder().id(2L).subject("Admin Test").build();
        Page<NotificationResponse> page = new PageImpl<>(Collections.singletonList(resp));
        when(notificationService.getAllNotifications(any(Pageable.class))).thenReturn(page);
