...
public class EmailService implements IEmailService {
    ...
    @Override
    public void sendGenericHtmlEmail(String toEmail, String subject, String htmlBody) {
        try {
            sendHtmlEmail(toEmail, subject, htmlBody);
            log.info("Generic HTML email sent to: {} with subject: {}", toEmail, subject);
            
            publishBusinessLog("EMAIL_SENT", "Generic HTML email sent successfully", Map.of(
                    "recipient", toEmail,
            ...
            log.error("Failed to send generic HTML email to: {}", toEmail, e);
            
            publishBusinessLog("EMAIL_FAILED", "Generic HTML email sending failed", Map.of(
                    "recipient", toEmail,
                    "emailType", "ADMIN_REPORT",
                    "subject", subject,
                    "deliveryStatus", "FAILED",
    ...
    public void sendEmailWithAttachment(String toEmail, String subject, String body, byte[] attachment, String fileName) {
        try {
            ...
            mailSender.send(message);
            log.info("Email with attachment sent to: {} with subject: {}", toEmail, subject);
            
            publishBusinessLog("EMAIL_SENT", "Email with attachment sent successfully", Map.of(
                    "recipient", toEmail,
                    "emailType", "ADMIN_REPORT_ATTACHMENT",
                    "subject", subject,
                    "deliveryStatus", "SENT"
            ));
        } catch (Exception e) {
            ...
            publishBusinessLog("EMAIL_FAILED", "Email with attachment sending failed", Map.of(
                    "recipient", toEmail,
                    "emailType", "ADMIN_REPORT_ATTACHMENT",
                    "subject", subject,
                    "deliveryStatus", "FAILED",
